...
func TestUserService_Update(t *testing.T) {
	...
	mock.ExpectQuery(`UPDATE users SET .+ WHERE id = \$5 AND version = \$6`).
		WithArgs("Updated User", "updated@example.com", sqlmock.AnyArg(), true, int64(1), 1).
	...
	err = user.Password.Set("newpassword123")
	if err != nil {
		t.Fatalf("failed to set password: %v", err)
	}
...
func TestUserService_Update_Errors(t *testing.T) {
	tests := []struct {
		name      string
		mockError error
		wantError error
	}{
		{"edit conflict", sql.ErrNoRows, ErrEditConflict},
		{"database error", sqlmock.ErrCancelled, sqlmock.ErrCancelled},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`UPDATE users SET .+ WHERE id = \$5 AND version = \$6`).
				WithArgs("Test User", "test@example.com", sqlmock.AnyArg(), false, int64(1), 1).
				WillReturnError(tt.mockError)
...
func TestUserService_Update_DuplicateEmail(t *testing.T) {
	...
	mock.ExpectQuery(`UPDATE users SET .+ WHERE id = \$5 AND version = \$6`).
		WithArgs("Test User", "duplicate@example.com", sqlmock.AnyArg(), false, int64(1), 1).
		WillReturnError(duplicateEmailErr)
